...
function loadAudioSettings() {
    ...
    if (!audioEl) return;
    const savedVol = localStorage.getItem('tm_monitor_volume');
    const savedMute = localStorage.getItem('tm_monitor_muted');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(btnMute) {
        btnMute.onclick = () => { if(audioEl) { audioEl.muted = !audioEl.muted; updateMuteUI(); localStorage.setItem('tm_monitor_muted', audioEl.muted); } };
    }
    if(volSlider && audioEl) {
        volSlider.oninput = (e) => {
            const val = e.target.value; audioEl.volume = val;
            if (val > 0 && audioEl.muted) { audioEl.muted = false; updateMuteUI(); localStorage.setItem('tm_monitor_muted', false); }
            localStorage.setItem('tm_monitor_volume', val); showVolTooltip(e, val);
        };
        if(volTooltip) {
...
function getVisibilitySettings() { const s = localStorage.getItem('tm_monitor_visibility'); return s ? JSON.parse(s) : {}; }
function saveVisibilitySettings(s) { localStorage.setItem('tm_monitor_visibility', JSON.stringify(s)); }
